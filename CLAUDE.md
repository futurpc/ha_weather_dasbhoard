...
- **Deploy script**: `./deploy.sh [build|flash|monitor|all|sim]` — wrapper that auto-detects PlatformIO path
  ...
  - `./deploy.sh sim` — build + run SDL simulator on Mac (no board needed)
- **Manual commands** (if not using deploy.sh):
  - Build: `pio run -d "<project_dir>" -e esp32s3`
  - Flash: `pio run -d "<project_dir>" -e esp32s3 -t upload`
  - Simulator: `pio run -e native && .pio/build/native/program`
...
## Project Structure
```
deploy.sh             - Build/flash/monitor/sim helper script
src/
  main.cpp            - Entry point: setup/loop, LVGL timer for 30s HA polling
  config.h            - Entity IDs, display dimensions; includes secrets.h (guarded by SIMULATOR)
  ...
  sim/
    sim_main.cpp      - SDL simulator entry point (native build only, guarded by SIMULATOR)
    include/
      Arduino.h       - Mock Arduino String class for native builds
include/
  lv_conf.h           - LVGL configuration (conditional: PSRAM vs malloc, Arduino vs SDL tick)
  lv_drv_conf.h       - lv_drivers SDL configuration (simulator only)
```

## Secrets / Credentials
- Sensitive data lives in `src/secrets.h` (gitignored) — WiFi SSID/password, HA base URL, HA token
...
## SDL Simulator
- **Purpose**: Preview the UI on Mac without flashing to hardware
- **Requires**: SDL2 (`brew install sdl2`)
- **Run**: `./deploy.sh sim` — builds `[env:native]` and launches the window
- **How it works**:
  - PlatformIO `[env:native]` builds for macOS instead of ESP32
  - `build_src_filter` includes only: `ui.cpp`, weather fonts, `sim/sim_main.cpp`
  - Mock `Arduino.h` in `src/sim/include/` provides a minimal `String` class
  - `lv_conf.h` uses `#ifdef SIMULATOR` to swap `ps_malloc` -> `malloc` and `millis()` -> `SDL_GetTicks()`
  - `config.h` guards `#include "secrets.h"` with `#ifndef SIMULATOR`
  - `lv_drv_conf.h` configures the SDL display driver (800x480 window)
  - `sim_main.cpp` populates mock weather data and renders the full dashboard UI
  - Mouse clicks in the SDL window act as touch input (F/C and theme toggles work)
- **Mock data**: Edit `make_mock_data()` in `src/sim/sim_main.cpp` to change displayed values
